...
public enum AlertKitAPI {
    ...
    public static func present(view: AlertViewProtocol, window: UIWindow? = nil, completion: @escaping ()->Void = {}) {
        guard let window = window ?? UIApplication.shared.windows.filter({ $0.isKeyWindow }).first else { return }
        view.present(on: window, completion: completion)
    }
    
    public static func present(title: String? = nil, subtitle: String? = nil, icon: AlertIcon? = nil, style: AlertViewStyle, haptic: AlertHaptic? = nil, window: UIWindow? = nil) {
        switch style {
        #if os(iOS)
